...
export default function App() {
  // Pelitila laajennettu sisältämään päävalikon ja eri pelimoottorit
  ...
  // Ohjataan vanhaan Kaartjärven 2D-pelimoottoriin
  ...
  const menuTitleStyle: React.CSSProperties = {
    fontSize: "32px", fontWeight: 900, color: "#ffffff", margin: "0 0 8px 0", letterSpacing: "-0.02em"
  };
      ...
      <div style={menuContainerStyle}>
        <div style={menuCardStyle}>
          ...
          <button 
            disabled={true}
            style={{
              width: "100%", padding: "16px", margin: "8px 0", backgroundColor: "#111827",
              border: "1px dashed rgba(255,255,255,0.1)", borderRadius: "12px", color: "#4b5563",
              fontSize: "14px", fontWeight: "bold", cursor: "not-allowed", display: "flex",
              flexDirection: "column", alignItems: "center", gap: "4px", opacity: 0.5
            }}
          >
          ...
          <button 
            ...
            style={{
              width: "100%", padding: "16px", margin: "8px 0", backgroundColor: isNameEmpty ? "#1e293b" : "#db2777",
              border: "none", borderRadius: "12px", color: "#ffffff", fontSize: "14px", fontWeight: "bold",
              cursor: isNameEmpty ? "not-allowed" : "pointer", display: "flex", flexDirection: "column",
              alignItems: "center", gap: "4px", opacity: isNameEmpty ? 0.4 : 1, transition: "all 0.15s ease",
              boxShadow: isNameEmpty ? "none" : "0 4px 14px rgba(219, 39, 119, 0.4)"
            }}
          >
            <span>🥽 Avaa VR-Pakohuone (Salakellari)</span>
            <span style={{ fontSize: "11px", fontWeight: "normal", color: isNameEmpty ? "#64748b" : "#fbcfe8" }}>Mikaelin salakellari PC:lle tai VR-laseille</span>
          </button>
          ...
          >
            <span>🔒 Osa 4: Kaartjärven Labyrintti</span>
            <span style={{ fontSize: "11px", fontWeight: "normal", color: "#374151" }}>Tulossa pian... (Monen huoneen pakopeli)</span>
          </button>
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#020617", color: "#cbd5e1" }}>
      {gameState === "lobby" && (
        <LobbyScreen
          playerId={playerId} playerName={playerName} setPlayerName={setPlayerName}
          gameCode={gameCode} setGameCode={setGameCode} isSoloMode={isSoloMode}
          setIsSoloMode={setIsSoloMode} onGameStarted={handleGameStarted}
        />
      )}

      {gameState === "reveal" && currentStage === "reveal" && (
        <RoleRevealScreen
          playerId={playerId} gameCode={gameCode} isSoloMode={isSoloMode}
          lobbyData={lobbyData} onNextStage={() => setGameState("investigation")}
        />
      )}
      ...
      {gameState === "voting" && (
        <VotingScreen
          playerId={playerId} gameCode={gameCode} isSoloMode={isSoloMode}
          lobbyData={lobbyData} resultsData={resultsData}
          onNextStage={() => {
            if (lobbyData) {
