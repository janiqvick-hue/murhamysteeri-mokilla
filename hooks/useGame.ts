...
export function useGame(code: string | null): UseGameReturn {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        ...
        if (!data) {
          setError("PeliÃ¤ ei lÃ¶ydy koodilla: " + code);
          setGame(null);
        } else {
          setGame(data);
          setError(null);
  ...
  const actions: GameActions = {
    ...
    joinGame: async (joinCode: string, playerName: string) => {
      ...
      if (!snap) throw new Error("PeliÃ¤ ei lÃ¶ydy koodilla: " + joinCode);
      if (snap.status !== "lobby")
        throw new Error("Peli on jo kÃ¤ynnissÃ¤. Et voi liittyÃ¤.");
    ...
    startNow: async () => {
      ...
      const playerIds = Object.keys(game.players ?? {});
      if (playerIds.length < 2) throw new Error("Tarvitaan vÃ¤hintÃ¤Ã¤n 2 pelaajaa.");
    ...
    sabotageClue: async (clueId: string, type: "remove" | "replace", fakeText: string) => {
      ...
      if (!clue) throw new Error("Vihje ei lÃ¶ydy.");
